...
class ExpandConv1x1(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, *, bias: bool=True, activation: str | nn.Module | None=None, dropout: float=0.0) -> None:
        super().__init__()
...
class PositionalEmbedding(nn.Module):
    ...
    def __init__(self, max_len: int, d_model: int, mode: str='learned') -> None:
        ...
        if self.mode == 'learned':
            ...
            div_term = torch.exp(torch.arange(0, self.d_model, 2, dtype=torch.float) * -(math.log(10000.0) / max(1, self.d_model - 1)))
